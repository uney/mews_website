<?php 
    ...
    if($stmt->rowCount()>0){
    	while ( $row = $stmt->fetch ( PDO::FETCH_ASSOC ) ) {
            ...
            $news_type = $row['news_type'];
            if($row['news_type']=="news_around"){
                $news_type_name = "身邊事";
            }else if($row['news_type']=="news_crime"){
                $news_type_name = "罪案";
		...
		<div class="primary">
			<article class="entry-item main-article">
				<header>
					...
					<span class="entry-news-info"><i id="news_upvote" onClick="voteForNews(this.id)" class="glyphicon glyphicon-fire"></i>:&nbsp;</span><span id="news_upvote_number"><?php echo $news_no_useful; ?>&nbsp;</span>
					<span class="entry-news-info"><i id="news_downvote" onClick="voteForNews(this.id)" class="fa fa-fw fa-fire-extinguisher"></i>:&nbsp;</span><span id="news_downvote_number"><?php echo $news_no_rubbish; ?>&nbsp;</span>
                    ...
                    echo "</span>";
                    echo "&nbsp;分類: <span class='entry-date'><a class='entry-category' href='search_result.php?search_type=type&search_key=".$news_type."'>".$news_type_name."</a></span>";
				...
				<?php
				    if($news_pic1!="" && strlen($news_pic1)>1){
				    	echo "<img class='responsive-img feature-img' src='../".GW_IMAGEPATH.$news_pic1."' alt='' />";
				    }
				?>
			...
			<section class="related-posts">
				...
				<ul class="clearfix">
                    ...
                    $mysql = "SELECT * FROM ".DB_TABLE_NEWS." WHERE (`news_type` ='".$news_type."'".$mysql_tag_query.") AND `news_id` <>".$news_id." ORDER BY RAND() LIMIT 5";
                    ...
                    while($row = $stmt->fetch ( PDO::FETCH_ASSOC )){
                        ...
                        if(strlen($row['news_pic1'])>1){
                            echo '<a href="primary.php?news_id='.$row['news_id'].'"><img class="responsive-img" src="../news_images/'.$row['news_pic1'].'" alt="" /></a>';
                        }
                        else{
